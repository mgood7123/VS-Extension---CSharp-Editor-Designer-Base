...
using Serilog;
using static Base.EditorFactories.XAML.XAML;
    ...
    internal partial class AvaloniaDesigner : UserControl, IDisposable
    {
        ...
        public bool IsPaused
        {
            ...
            set
            {
                if (_isPaused != value)
                {
                    Log.Logger.Debug("AndroidUI - AndroidUI - Setting pause state to {State}", value);
        ...
        {
            Log.Logger.Verbose("AndroidUI - Started AvaloniaDesigner.Start()");
            ...
            Log.Logger.Verbose("AndroidUI - Finished AvaloniaDesigner.Start()");
        }
        ...
        {
            Log.Logger.Verbose("AndroidUI - Started AvaloniaDesigner.LoadTargetsAndStartProcessAsync()");
            ...
            Log.Logger.Verbose("AndroidUI - Finished AvaloniaDesigner.LoadTargetsAndStartProcessAsync()");
        }

        private async Task LoadTargetsAsync()
        {
            Log.Logger.Verbose("AndroidUI - Started AvaloniaDesigner.LoadTargetsAsync()");

            _loadingTargets = true;

            try
            {
                var projects = await VS_Extension___CSharp_Editor_Designer_Base.VS_Extension___CSharp_Editor_Designer_BasePackage.SolutionService.GetProjectsAsync();
            ...
            Log.Logger.Verbose("AndroidUI - Finished AvaloniaDesigner.LoadTargetsAsync()");
        }
        ...
        {
            Log.Logger.Verbose("AndroidUI - Started AvaloniaDesigner.StartProcessAsync()");
            ShowPreview();
            ...
            if (assemblyPath != null && executablePath != null && hostAppPath != null)
            {
                ...
                catch (ApplicationException ex)
                {
                    ...
                    Log.Logger.Debug(ex, "AndroidUI - Process.StartAsync exited with error");
                }
                catch (FileNotFoundException ex)
                {
                    ShowError("Build Required", ex.Message);
                    Log.Logger.Debug(ex, "AndroidUI - StartAsync could not find executable");
                }
                catch (Exception ex)
                {
                    ShowError("Error", ex.Message);
                    Log.Logger.Debug(ex, "AndroidUI - StartAsync exception");
                }
                finally
                {
            ...
            {
                Log.Logger.Error("AndroidUI - No executable found");
            ...
            Log.Logger.Verbose("AndroidUI - Finished AvaloniaDesigner.StartProcessAsync()");
        }
            ...
            XamlBufferMetadata target)
        {
            ...
            Log.Logger.Verbose("AndroidUI - Started AvaloniaDesigner.CreateCompletionMetadataAsync() for {ExecutablePath}", executablePath);
            ...
            try
            {
                ...
                if (!_metadataCache.TryGetValue(executablePath, out metadataLoad))
                {
                    metadataLoad = Task.Run(() =>
                                    {
                                        var metadataReader = new Base.EditorFactories.XAML.MetadataReader(new DnlibMetadataProvider());
                ...
                sw.Stop();
                Log.Logger.Verbose("AndroidUI - Finished AvaloniaDesigner.CreateCompletionMetadataAsync() took {Time} for {ExecutablePath}", sw.Elapsed, executablePath);
            }
            catch (Exception ex)
            {
                Log.Logger.Error(ex, "AndroidUI - Error creating XAML completion metadata");
            }
            finally
            {
                Log.Logger.Verbose("AndroidUI - Finished AvaloniaDesigner.CreateCompletionMetadataAsync()");
            }
        }
        ...
        private async Task SelectedTargetChangedAsync(object sender, DependencyPropertyChangedEventArgs e)
        {
            ...
            Log.Logger.Debug(
                "AndroidUI - AvaloniaDesigner.SelectedTarget changed from {OldTarget} to {NewTarget}",
            ...
            if (oldValue?.ExecutableAssembly != newValue?.ExecutableAssembly)
            {
                if (_isStarted)
                {
                    try
                    {
                        Log.Logger.Debug("AndroidUI - Waiting for StartProcessAsync to finish");
                        await _startingProcess.WaitAsync();
                        Process.Stop();
